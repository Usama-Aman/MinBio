...
import com.vic.vicwsp.Views.Activities.CoverageSaveCards;

import java.util.ArrayList;

public class CoverageCardsAdapter extends RecyclerView.Adapter<CoverageCardsAdapter.ViewHolder> {

    private Context context;
    private ArrayList<Datum> saveCardsArrayList;
    private CoverageSaveCards coverageSaveCards;
    private AlertDialog alertDialog;

    public CoverageCardsAdapter(ArrayList<Datum> saveCardsArrayList, CoverageSaveCards coverageSaveCards) {
        this.saveCardsArrayList = saveCardsArrayList;
        this.coverageSaveCards = coverageSaveCards;
    }

    @NonNull
    @Override
    public CoverageCardsAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        context = parent.getContext();
        return new ViewHolder(LayoutInflater.from(parent.getContext()).inflate(R.layout.item_saved_cards, parent, false));
    }

    @Override
    public void onBindViewHolder(@NonNull CoverageCardsAdapter.ViewHolder holder, int position) {
        ...
        holder.cardsConstraint.setOnClickListener(v -> {
            coverageSaveCards.doPayment(saveCardsArrayList.get(position).getCardId());
        });
        ...
        holder.imgDeleteCard.setOnClickListener(v -> {
            ...
            alertDialog = new AlertDialog.Builder(context)
                    ...
                    .setPositiveButton(context.getResources()
                        ...
                        coverageSaveCards.deleteCard(saveCardsArrayList.get(position).getId(), position);
                    })
