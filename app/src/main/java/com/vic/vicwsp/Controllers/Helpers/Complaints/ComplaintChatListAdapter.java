...
public class ComplaintChatListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private ArrayList<ComplaintChatData> compliantsListData;

    public ComplaintChatListAdapter(Context context, ArrayList<ComplaintChatData> compliantsListData) {
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        if (compliantsListData.get(position).getCommentFrom().equals("user")) {
            if (AppUtils.isSet(compliantsListData.get(position).getUserDetail().getFullName())) {
                ((Items) holder).messageName.setText(compliantsListData.get(position).getUserDetail().getFullName());
            }

            if (AppUtils.isSet(compliantsListData.get(position).getUserDetail().getImagePath())) {
                Glide.with(context).load(compliantsListData.get(position).getUserDetail().getImagePath()).into(((Items) holder).userImage);
            }
        } else {
            if (AppUtils.isSet(compliantsListData.get(position).getAdminDetail().getName())) {
        ...
        if (AppUtils.isSet(compliantsListData.get(position).getCreatedAt())) {
            ((Items) holder).messageDate.setText(AppUtils.formatDateFromDateTime(compliantsListData.get(position).getCreatedAt()));
        }

        ((Items) holder).textMessage.setText("");
        if (AppUtils.isSet(compliantsListData.get(position).getComment())) {
            ((Items) holder).textMessage.setText(compliantsListData.get(position).getComment());
        }

        if (AppUtils.isSet(compliantsListData.get(position).getImagePath())) {
