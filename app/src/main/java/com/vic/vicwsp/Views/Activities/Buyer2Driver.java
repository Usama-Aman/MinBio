...
import com.vic.vicwsp.Controllers.Helpers.ChatAdapter;
import com.vic.vicwsp.Controllers.Interfaces.Api;
import com.vic.vicwsp.Models.Response.Chat.ChatModel;
import com.vic.vicwsp.Models.Response.Chat.Datum;
...
import static com.vic.vicwsp.Utils.Constants.AUDIO_REQUEST_CODE;
...
public class Buyer2Driver extends AppCompatActivity implements View.OnClickListener {
    ...
    private ImageView chatBack, ivListNull, headerLogo;
    ...
    public static int buyerDriverOrderId = 0;

    public static boolean isb2dChatActive = false;
    private boolean isSendingMessage = false;
    private BroadcastReceiver chatBroadcastReceiver = new BroadcastReceiver() {
        @Override
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction().equals("Buyer2DriverBroadCast")) {
                try {
                    JSONObject jsonObject = new JSONObject(intent.getStringExtra("messageData"));


                    if (buyerDriverOrderId == jsonObject.getInt("order_id")) {

                        String orderNumber = intent.getStringExtra("orderNumber");
//                        chatOrderNumber.setText(getResources().getString(R.string.chatOrderNumber, orderNumber));
    ...
    private TextView chatOrderNumber;


    @Override
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_chat_b2d);
        LocalBroadcastManager.getInstance(Buyer2Driver.this).registerReceiver(chatBroadcastReceiver, new IntentFilter("Buyer2DriverBroadCast"));
        isb2dChatActive = true;

        buyerDriverOrderId = getIntent().getIntExtra("orderId", 0);

        if (buyerDriverOrderId > 0) {
            initViews();
            setAdapter();
        } else {
            Common.showToast(Buyer2Driver.this, getResources().getString(R.string.something_is_not_right), false);
    ...
    private void initViews() {
        ...
        try {
            ...
            chatOrderNumber = findViewById(R.id.chatOrderNo);
//            chatOrderNumber.setText(getResources().getString(R.string.chatOrderNumber, getIntent().getStringExtra("orderNumber")));
    ...
    private void setAdapter() {
        chatAdapter = new ChatAdapter(Buyer2Driver.this, datumArrayList);
        layoutManager = new LinearLayoutManager(Buyer2Driver.this);
        ...
        Common.showDialog(Buyer2Driver.this);
    ...
    private void settingRecordViewListener() {
        ...
        recordView.setOnRecordListener(new OnRecordListener() {
            ...
            public void onStart() {
                ...
                if (ContextCompat.checkSelfPermission(Buyer2Driver.this, Manifest.permission.RECORD_AUDIO) == PackageManager.PERMISSION_DENIED
                        || ContextCompat.checkSelfPermission(Buyer2Driver.this, Manifest.permission.WRITE_EXTERNAL_STORAGE) == PackageManager.PERMISSION_DENIED) {
                    ActivityCompat.requestPermissions(Buyer2Driver.this, new String[]{Manifest.permission.RECORD_AUDIO, Manifest.permission.WRITE_EXTERNAL_STORAGE}, 2003);
                } else {
            ...
            public void onCancel() {
                ...
                if (ContextCompat.checkSelfPermission(Buyer2Driver.this, Manifest.permission.RECORD_AUDIO) == PackageManager.PERMISSION_GRANTED
                        || ContextCompat.checkSelfPermission(Buyer2Driver.this, Manifest.permission.WRITE_EXTERNAL_STORAGE) == PackageManager.PERMISSION_GRANTED) {
                    shutDownRecorder();
                }
            }
    ...
    public void checkPermission(String permission, int requestCode) {
        ...
        if (ContextCompat.checkSelfPermission(Buyer2Driver.this, permission) == PackageManager.PERMISSION_DENIED) {
            ActivityCompat.requestPermissions(Buyer2Driver.this, new String[]{permission}, requestCode);
        } else {
    ...
    private void getMessageFromApi(int c) {
        ...
        Call<ChatModel> call = api.getBuyerDriverMessages("Bearer " + SharedPreference.getSimpleString(Buyer2Driver.this, Constants.accessToken),
                buyerDriverOrderId, c);
        ...
        call.enqueue(new Callback<ChatModel>() {
                ...
                try {
                    if (response.isSuccessful()) {
                        ...
                        if (response.body().getData().size() > 0) {
                            ...
                            chatOrderNumber.setText(getResources().getString(R.string.chatOrderNumber, response.body().getParentOrderId()));
                        ...
                        Common.showToast(Buyer2Driver.this, jsonObject.getString("message"), false);
                        showNullMessage();
                    }
                } catch (Exception e) {
                    e.printStackTrace();
            ...
            public void onFailure(Call<ChatModel> call, Throwable t) {
                Common.dissmissDialog();
                Log.d("Buyer2Driver", "onFailure: " + t.getMessage());
                showNullMessage();
            }
        });
    ...
    private void postMessage(String message, File file, String type, String duration) {
        ...
        RequestBody rbOrderId = RequestBody.create(MediaType.parse("text/plain"), String.valueOf(buyerDriverOrderId));
        ...
        if (type.equals("text")) {
            ...
            call = api.postTextMessageBuyerDriver("Bearer " + SharedPreference.getSimpleString(Buyer2Driver.this, Constants.accessToken),
            ...
            call = api.postFileMessageBuyerDriver("Bearer " + SharedPreference.getSimpleString(Buyer2Driver.this, Constants.accessToken),
    ...
    protected void onDestroy() {
        super.onDestroy();
        LocalBroadcastManager.getInstance(Buyer2Driver.this).unregisterReceiver(chatBroadcastReceiver);
        stopAudio();
    }
